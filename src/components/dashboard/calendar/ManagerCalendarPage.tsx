...
import { Target, Calendar as CalendarIcon, Clock, X, MoreHorizontal, Users } from 'lucide-react';
...
export const ManagerCalendarPage: React.FC = () => {
  const { teamTasks, teamGoals, teamMembers, loading } = useManagerData();
  const [selectedEvent, setSelectedEvent] = useState<any | null>(null);

  const events = [
    ...teamTasks.filter(t => t.deadline).map(t => ({
      ...t,
      title: `${teamMembers.find(m => m.uid === t.userId)?.displayName || 'User'}: ${t.title}`,
      start: t.deadline!.toDate(),
      end: t.deadline!.toDate(),
      allDay: true,
      type: 'task'
    })),
    ...teamGoals.filter(g => g.targetDate).map(g => ({
      ...g,
      title: `${teamMembers.find(m => m.uid === g.userId)?.displayName || 'User'}: ${g.title}`,
  ...
  if (loading) return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-slate-400 font-bold uppercase tracking-widest text-xs animate-pulse">Synchronizing Team Schedule...</p>
      </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      <header className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Workforce Calendar</h2>
            <p className="text-slate-500 text-sm mt-2 font-medium">Unified landscape of team commitments and strategic deadlines.</p>
        </div>
        <div className="flex gap-4">
            <div className="flex -space-x-2">
                {teamMembers.slice(0, 5).map(m => (
                    <div key={m.uid} className="w-8 h-8 rounded-full bg-slate-100 border-2 border-white flex items-center justify-center text-[10px] font-black text-slate-600" title={m.displayName}>
                        {m.displayName[0]}
                    </div>
                ))}
                {teamMembers.length > 5 && (
                    <div className="w-8 h-8 rounded-full bg-indigo-600 border-2 border-white flex items-center justify-center text-[10px] font-black text-white">
                        +{teamMembers.length - 5}
                    </div>
                )}
            </div>
            <button className="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">Team Analytics</button>
        </div>
      </header>
            ...
            >
                ...
                <div className="relative z-10">
                    ...
                    <div className="mt-10 flex gap-4">
                        <button className="flex-1 bg-slate-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-800 transition-all shadow-xl shadow-slate-200">Open Profile</button>
